...
output "db_endpoint" {
  ...
  value       = local.create_database ? replace(aws_db_instance.database[0].endpoint, ":${aws_db_instance.database[0].port}", "") : ""
}

output "db_username" {
  description = "The username for the RDS database"
  value       = local.create_database ? var.db_username : ""
}

output "db_password" {
  description = "The password for the RDS database"
  value       = local.create_database ? var.db_password : ""
  sensitive   = true # Prevents the password from being shown in plain text in the output
}

output "db_port" {
  description = "The port for the RDS instance"
  value       = local.create_database ? aws_db_instance.database[0].port : ""
}

output "db_connection_string" {
  description = "The full database connection string"
  value       = local.create_database ? "${local.db_scheme}://${var.db_username}:[YOUR-PASSWORD]@${aws_db_instance.database[0].endpoint}/${var.db_name}" : ""
}
